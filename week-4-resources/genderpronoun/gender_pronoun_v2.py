...
BUCKET = 'bucketsparkweek2' # name of your bucket
...
PGCAT = WORKINGDIR + 'pgcat-cluster.json/part-00000' # path to your catalogue index (expects json)
bind(DCTERMS.RFC4646, str)


def cnt_pronouns(id,text):
    """ Count gender pronouns in a single ebook """
    mcount = len(re.findall(r'\b(he|him|himself|his)\b', str(text)))
    fcount = len(re.findall(r'\b(she|her|herself|hers)\b', str(text)))
    ...
    if bucket_name in buckets:
        ...
        for x in range(1,10):
            response = s3_client.list_objects(
                    Bucket=bucket_name,
                    Prefix='gutenberg_dataset/'+str(x)+'/',
                    )
